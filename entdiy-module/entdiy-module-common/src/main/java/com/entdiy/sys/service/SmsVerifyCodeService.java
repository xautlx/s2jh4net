...
public class SmsVerifyCodeService extends BaseService<SmsVerifyCode, Long> {
    ...
    public String generateSmsCode(HttpServletRequest request, String mobileNum, boolean mustExist) {
        ...
        if (smsVerifyCode == null) {
            //要求手机号之前必须已经成功验证过
            if (mustExist && !AppContextHolder.isDevMode()) {
            ...
            //已过期
            if (DateUtils.currentDateTime().isAfter(smsVerifyCode.getExpireTime())) {
                updateCode = true;
            }
        }
        ...
        if (updateCode) {
            ...
            smsVerifyCode.setGenerateTime(DateUtils.currentDateTime());
            //5分钟有效期
            smsVerifyCode.setExpireTime(smsVerifyCode.getGenerateTime().plusMinutes(VERIFY_CODE_LIVE_MINUTES));
    ...
    public boolean verifySmsCode(HttpServletRequest request, String mobileNum, String code) {
        ...
        //验证码已过期
        if (smsVerifyCode.getExpireTime().isBefore(now)) {
            return false;
        }
    ...
    public void removeExpiredDataTimely() {
        ...
        if (smsVerifyCodeDao.countTodoItems() > 0) {
            int effectiveCount = smsVerifyCodeDao.batchDeleteExpireItems(DateUtils.currentDateTime());
