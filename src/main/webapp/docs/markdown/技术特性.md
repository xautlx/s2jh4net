...
自己也是两种类型的项目都多次经历，经过权衡考虑，最终选择了JPA+Hibernate为主，MyBatis为辅的组合，几点理由：
...
同时框架也引入了MyBatis作为辅助的数据访问，主要是考虑到一些复杂的图标，报表等统计查询功能，直接采用MyBatis方式会更加灵活。但是切记注意：由于本框架优先采用Hibernate进行数据管理，因此MyBatis仅用于复杂的查询之用，不做数据更新操作以免干扰Hibernate缓存。
